...
# remove redundant columns or repeated columns
...
# remove more redunctant, repeated columns
a = names(df_earthquake)
df_earthquake=df_earthquake %>% select(all_of(a[!grepl("_query", a)]))
...
feat_median=sapply(df_earthquake, function(x) {
  if (is.numeric(x)) {
    round(quantile(x, probs = 0.5, na.rm = TRUE),2)
  } else {
    NA
  }
})
...
variable_summ=data.frame(feature=names(df_earthquake), 
                         ...
                         q75=unname(feat_q75),
                          `max`=unname(feat_max),
                         `sd`=round((unname(feat_var))**0.5,2)
                         )
...
corr_hm=corr_data_df %>% ggplot(aes(x=Var2, y=Var1, fill=value))+
geom_tile() +
...
![Feature Correlation Matrix on subset of features](corr_heatmap.png){width="341" height="199"}
...
#| fig-width: 8
#| paged-print: false
#| fig-cap: "Earthqake Measurement Sensitivity Over Time"


tot=df_earthquake %>% distinct(id,date_final,nst,richter_scale) %>% 
  filter(!is.na(nst)) %>% 
  mutate(year_=year(date_final)) %>% 
  group_by(year_) %>% 
  summarize(nst_cnt=mean(nst,na.rm=T)) %>% 
  mutate(richter_scale='Total')

scale_=  df_earthquake %>% distinct(id,date_final,nst,richter_scale) %>% 
  filter(!is.na(nst)) %>% 
  mutate(year_=year(date_final)) %>% 
  group_by(year_,richter_scale) %>% 
  summarize(nst_cnt=mean(nst,na.rm=T)) %>% 
  select(year_,nst_cnt,richter_scale)

group_colors = c("Total" = "black", "Moderate" = "pink", "Strong" = "green", 'Major'='steelblue')


nst_p=rbind(tot,scale_) %>% mutate(richter_scale=factor(richter_scale,levels=c("Total","Moderate","Strong",'Major')    )) %>% 
  ggplot(aes(x=year_,y=nst_cnt,color=richter_scale))+
  geom_line()+
  geom_point()+
  labs(title = "Yearly Avg Number of Seismic Stations (NST) Per ID",y='avg nst count',x = "year",color='scale') +
  scale_color_manual(values = group_colors)+
  theme_minimal()


tot=df_earthquake %>% distinct(id,date_final,rms,richter_scale) %>% filter(!is.na(rms)) %>% 
  mutate(year_=year(date_final)) %>% 
  group_by(year_) %>% 
  summarize(mean_rms=mean(rms,na.rm=T)) %>% 
  mutate(richter_scale='Total')

scale_=  df_earthquake %>% distinct(id,date_final,rms,richter_scale) %>% 
  filter(!is.na(rms)) %>% 
  mutate(year_=year(date_final)) %>% 
  group_by(year_,richter_scale) %>% 
  summarize(mean_rms=mean(rms,na.rm=T)) %>% 
  select(year_,mean_rms,richter_scale)

group_colors = c("Total" = "black", "Moderate" = "pink", "Strong" = "green", 'Major'='steelblue','Great'='orange')


rms=rbind(tot,scale_) %>% mutate(richter_scale=factor(richter_scale,levels=c("Total","Moderate","Strong",'Major', "Great")    )) %>% 
  ggplot(aes(x=year_,y=mean_rms,color=richter_scale))+
  geom_line()+
  geom_point()+
  labs(title = "Yearly Avg Root Mean Sqaure Error Per ID",y='avg rms',x = "year",color='scale') +
  scale_color_manual(values = group_colors)+
  theme_minimal()


tot=df_earthquake %>% distinct(id,date_final,magnitudeerror,richter_scale) %>% filter(!is.na(magnitudeerror)) %>% 
  mutate(year_=year(date_final)) %>% 
  group_by(year_) %>% 
  summarize(mean_magnitudeerror=mean(magnitudeerror,na.rm=T)) %>% 
  mutate(richter_scale='Total')

scale_=  df_earthquake %>% distinct(id,date_final,magnitudeerror,richter_scale) %>% 
  filter(!is.na(magnitudeerror)) %>% 
  mutate(year_=year(date_final)) %>% 
  group_by(year_,richter_scale) %>% 
  summarize(mean_magnitudeerror=mean(magnitudeerror,na.rm=T)) %>% 
  select(year_,mean_magnitudeerror,richter_scale)

group_colors = c("Total" = "black", "Moderate" = "pink", "Strong" = "green", 'Major'='steelblue','Great'='orange')


mg_err=rbind(tot,scale_) %>% mutate(richter_scale=factor(richter_scale,levels=c("Total","Moderate","Strong",'Major', "Great")    )) %>% 
  ggplot(aes(x=year_,y=mean_magnitudeerror,color=richter_scale))+
  geom_line()+
  geom_point()+
  labs(title = "Yearly Avg Root Mean Sqaure Error Per ID",y='avg magnitude error',x = "year",color='scale') +
  scale_color_manual(values = group_colors)+
  theme_minimal()
...
rbind(tot,scale_) %>% mutate(richter_scale=factor(richter_scale,levels=c("Total","Moderate","Strong",'Major', "Great")    )) %>% 
  ggplot(aes(x=year_,y=mean_magnitudeerror,color=richter_scale))+
  geom_line()+
  geom_point()+
  labs(title = "Yearly Avg Root Mean Sqaure Error Per ID",y='avg magnitude error',x = "year",color='scale') +

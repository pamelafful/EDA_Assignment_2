...
query %>% filter(is.na(dt_date_time))
```
...
earth_quakes_final %>% filter(is.na(dt_date_time))
```
...
df_earthquake %>% filter(is.na(dt_date_time)) %>% distinct(id)

df_earthquake %>% filter(id=='us7000jkwd') %>% select(id,dt_date_time,dt_date_time_query) 
...
#   filter(is.na(year_))
# 
# df_earthquake %>% filter(is.na(year(dt_date_time))) %>% 
#   select(dt_date_time)
...
```{r,loc_ext,eval=TRUE, echo=FALSE}
...
```{r,countries_plot,eval=TRUE, echo=FALSE}

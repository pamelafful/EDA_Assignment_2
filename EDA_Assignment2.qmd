...
View(sdf_earthquake_countries %>% select(id,place,location,name) %>% filter(is.na(name) ))

sdf_earthquake %>% mutate(country=case_when(
  ...
  filter( country) %>% slice(n=20)
  ggplot(aes(x=country,y=eq_count))+
  geom_col(stat = 'identity')+

...
#df_earthquake %>% filter(!is.na(place)) %>% head() %>% select(place)
...
```{r}
# map id with # na values in the place feature with inbuit dataset
world_countries <- ne_countries(scale = "medium", returnclass = "sf")

wc=world_countries %>% select(name,geometry) %>% rename(country_=name)
na_country=st_join(sdf_earthquake,wc,join=st_intersects,left=T) %>% 
  filter(is.na(country_)) %>% distinct(place,location)

st_join(sdf_earthquake,wc,join=st_intersects,left=T) %>% 
  mutate(country_fill=coalesce(country_,location)) %>% 
  mutate(location_fill=coalesce(location,country_)) %>% 
  mutate(country_final= coalesce(country_fill,location_fill)) %>% 
  mutate(country_final=coalesce(country_final,'Unknown'))
  select(id,place,country_,location,country_,country_final) %>% 
  


mp=leaflet(na_country) %>%
  addTiles() %>%
  addCircleMarkers(
    radius = ~magnitude,   
    color = "purple",       

...
\~ 95% (\~32.5K) of IDs have missing values for horizontal depth. Of there remaining 5% ( \~1.5K), the average horizontal distance is highest for moderate earthquake types at 4.33 followed by strong earthquakes at 3.38m\
...
Since 2017, the number of recorded earthquakes has sharply increased, peaking in 2021 with 2,200 occurrences, representing a 135% increase from the previous year- although the average recorded magnitudes showed relative decreases, with the average scale within the moderate scale. The sudden increase also occurs around the same period in which the average number of seismic stations per ID (occurrence) peaking at approximately 112 stations per earthquake ID in 2022; Lower magnitude errors[^1] and root mean square errors[^2] were also observed during this time, indicating improved earthquake detection and measurement sensitivity.

[^1]: A statistical measure used to evaluate how well a model predicts earthquake magnitudes.

[^2]: The uncertainty or difference in the measured earthquake magnitude.
...
-   Use the SpaCy Named Entity Recognition (NER) function to extract locations from the "place" column (where available) and save these extracted values in a dataframe/csv file named "location_entities." Then, join this data back to the \`df_earthquake\` dataFrame using the **ID** key. After joining, convert the resultant dataframe into a spatial dataframe.
-   Next, use the built-in **countries** spatial dataframe from the rnaturalearth package, which contains a list of countries and their geometries. Perform a spatial join with \`sdf_earthquake\` using the **st_is_within_distance** predicate, which joins geometries that are within a specified distance of each other. In this case, apply a threshold of 100 kilometers, allowing country locations to be tagged to earthquake IDs that occurred within 100 kilometers of a country's geometry.
-   Finally, tag the location of any IDs that could not be extracted using the aforementioned methods as "Unknown."
...
#| fig-width: 6
#| paged-print: false
#| fig-cap: "Count of Location-Tagged Earthquake IDs"
...
In terms of the global magnitude distributions, the highest average recorded magnitudes (magnitudes \>\~6) are mostly clusterd in South America and Asia. North America and Asia report the highest counts of earthquake incidents in the recorded period.

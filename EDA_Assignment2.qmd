...
Although the time span of the data is quite extensive, the dataset is significantly sparse across many attributes and observations, with regional and country locations missing for many observations. To enhance the dataset, two key features have been added. The first is the "Scale" feature, which categorises earthquake types into magnitude buckets using definitions from the Richter Scale(reference needed). The second involves location extraction techniques that provide approximate country and regional labels for different IDs. This is accomplished through spatial joins using built-in data from the rnaturalearth package, as well as utilising Named Entity Recognition from the Spacy package to extract labelled country data directly from the dataset. Exploratory insights are divided into three parts:
...
8.  Create richter_scale column using the category definition from [@usgs_magnitude]. According to the USGS website the magnitude types give the same magnitudes for earthquakes with magnitudes ranging from 6.5 to 7.0. In the combined earthquake dataframe, earthquakes there are 817 earthquake ids that exceed 7, accounting for \~2% of the data. As such, the magnitudes scale categories are defined solely based off the nominal values of the magnitude values themselves.
...
dt_summ=kable(na.omit(variable_summ), caption = 'Cleaned Earthquake Dataset Numeric Features Data Summary', format = "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = c("hold_position"))


```

![](images/clipboard-1787642721.png){fig-align="left" width="637" height="322"}
...
Earthquake are mostly distributed above/ to the right of the median earthquake magnitudes (5.6,moderate); \~60% of occurrences were above the median, whilst \~30% of ids lie above the 75th percentile (5.9) of magnitudes. Only 2% of recorded earthquakes had a magnitude greater than 7.
...
p2=sdf_earthquake %>% 
  ...
  geom_col(stat = 'identity')+
  labs(title = "Top 20 Countries By Total Number of Earthquke Occurances (1965-2023) ",y='#occurances',
...
#| paged-print: false
#| label: fig-barplots8
#| fig-cap: "12 month average earthquake occurance per country, Colored By Average Magnitude"
#| 
...
p2_1=country_eq %>% group_by(country_final,year_) %>% 
  ...
  geom_col()+
   scale_fill_gradient(low = "yellow", high = "red")+
    labs(title = "Top 20 12 Month Average Earthquake Occurances Per Country ",
         x='country',y= "mean #occurances",fill = "mean magnitude")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 1))
...
bc=sdf_earthquake %>% group_by(continent) %>%filter(!is.na(continent)) %>% 
...
![Global Earthquake Occurrences (1965-2024)](eq_global_distribution.png){width="762"}

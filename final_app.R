...
continent_options=sdf_eq %>% filter(!is.na(continent)) %>%  
  distinct(continent) %>% pull()


year_options=unique( year(sdf_eq$date_final))
...
ui = dashboardPage(
  ...
  dashboardBody(
    ...
    # Second row - Table and Hourly Distribution
    fluidRow(
      box(
        ...
        fluidRow(
          column(
            width = 6,
            # Individual country filter for the table
            pickerInput(
              "table_year_filter",
              "Filter Year:",
              choices = year_options,
              multiple = TRUE,
              selected = year_options,
              options = list(
                `actions-box` = TRUE,
                `live-search` = TRUE,
                `selected-text-format` = "count > 3",
            ...
            DTOutput("top_countries_table", height = "280px")
          ),
          column(
            width = 6,
            pickerInput(
            ...
            pickerInput(
              "hrly_region_filter",
              "Filter Region:",
              choices = continent_options,  # Changed from country_options
              multiple = TRUE,
              selected = continent_options,  # Changed from country_options
              options = list(
                `actions-box` = TRUE,
                `live-search` = TRUE,
                `selected-text-format` = "count > 3",
...
server = function(input, output, session) {
    ...
    joy_filtered_data <- reactive({
      req(input$table_year_filter, input$hrly_country_filter)
      sdf_eq %>%
        filter(
          year_==input$table_year_filter,
          country_final %in% input$hrly_country_filter,
          continent %in% input$hrly_region_filter
        )
    }) %>% debounce(300) 
    ...
    ind_filtered_data <- reactive({
      req(input$Country_Filter, input$Scale_Filter, input$Continent_Filter)
        ...
        filter(richter_scale %in% input$ts_scale_filter, 
                 country_final %in% input$ts_country_filter,
                 continent %in% input$ts_region_filter)
    }) %>% debounce(300)
    
 
    bb_filtered_data <- reactive({  
      req(input$bubble_scale_filter, input$bubble_year_filter)
      
      sdf_eq %>% 
        filter(richter_scale %in% input$bubble_scale_filter, 
               year_ == input$bubble_year_filter)
    }) %>% debounce(300)  
    
    
    top_5_filtered_data=reactive({
        sdf_eq %>%
          filter(year(date_final) ==input$table_year_filter)
      })
